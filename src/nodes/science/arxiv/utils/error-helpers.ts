...
export function handleApiError(error: unknown): string {
  ...
  if (error && typeof error === 'object' && 'status' in error) {
    ...
    if (fetchError.status === 429) {
      logger.error(LogCategory.NODE, '[ArxivAPI]', 'Rate limit exceeded', {
        error: fetchError
      });
    ...
    if (fetchError.status === 404) {
      logger.error(LogCategory.NODE, '[ArxivAPI]', 'Resource not found', {
        error: fetchError
      });
    ...
    if (fetchError.status >= 500) {
      logger.error(LogCategory.NODE, '[ArxivAPI]', 'Server error', {
        error: fetchError
      });
    ...
    const statusText =
      fetchError.statusText || `HTTP error ${fetchError.status}`;
  ...
  if (error && typeof error === 'object' && 'xmlCode' in error) {
    ...
    logger.error(LogCategory.NODE, '[ArxivAPI]', 'XML parsing error', {
      error: xmlError
    });
  ...
  logger.error(LogCategory.NODE, '[ArxivAPI]', 'Unknown error type', {
    error: errorStr
  });
