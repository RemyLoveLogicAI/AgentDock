...
const createMockInput = (
  ...
  return {
    ...
    criteria: criteria || [
      { name: 'SentimentTest', description: 'Test', scale: 'numeric' }
    ]
  };
};
...
describe('SentimentEvaluator', () => {
  ...
  describe('Constructor Error Handling', () => {
    it('should throw if criterionName is missing', () => {
      ...
      expect(
        () => new SentimentEvaluator(config as SentimentEvaluatorConfig)
      ).toThrow(
  ...
  describe('Core Sentiment Analysis', () => {
    const positiveText =
      'This is a wonderful and amazing experience! Great job!'; // Should be positive
    ...
    it('should output normalized comparative score (0-1) by default', async () => {
      ...
      const resultsPositive = await evaluator.evaluate(
        createMockInput(positiveText),
        [mockCriterionNumeric]
      );
      const resultsNegative = await evaluator.evaluate(
        createMockInput(negativeText),
        [mockCriterionNumeric]
      );
      const resultsNeutral = await evaluator.evaluate(
        createMockInput(neutralText),
        [mockCriterionNumeric]
      );
  ...
  describe('Advanced Field Sourcing', () => {
    const positiveText =
      'This is a wonderful and amazing experience! Great job!';
    ...
    it('should source text from groundTruth (string) via sourceTextField', async () => {
      ...
      expect(results[0].reasoning).toContain(
        "Sentiment analysis of 'groundTruth'"
      );
    });
