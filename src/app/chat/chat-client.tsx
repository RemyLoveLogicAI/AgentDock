...
function ChatPageContent() {
  ...
  const agentName = agentId
    ? templates[agentId as TemplateId]?.name
    : undefined;
  ...
  const handleStateUpdate = useCallback(
    (state: ChatContainerState) => {
      ...
      if (state.orchestration) {
        ...
        if (
          state.orchestration?.sessionId &&
          state.orchestration.sessionId !== currentSessionId
        ) {
...
export default function ChatClientPage() {
  ...
  const fallback = isFirstLoad ? (
    ...
    <ChatSkeleton />
  ); // Provide a default skeleton if not first load but still suspending
