 ...
 */
function processAuthorId(authorId: string): {
  id: string;
  type: 'authorId' | 'orcid';
} {
  // ORCID pattern (e.g., 0000-0002-1825-0097)
  if (
    authorId.match(/^\d{4}-\d{4}-\d{4}-\d{4}$/) ||
    authorId.toLowerCase().startsWith('orcid:')
  ) {
...
export async function fetchSemanticScholarAuthor(
  ...
  try {
    ...
    const author: SemanticScholarAuthor = {
      ...
      url:
        data.url || `https://www.semanticscholar.org/author/${data.authorId}`,
    ...
    throw new Error(
      `Error fetching author from Semantic Scholar: ${errorMessage}`
    );
  }
}
