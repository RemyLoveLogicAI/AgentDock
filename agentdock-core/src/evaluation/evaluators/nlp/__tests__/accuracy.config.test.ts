...
describe('NLPAccuracyEvaluator - Configuration Options', () => {
  ...
  it('should use custom embeddingModel if provided in config', async () => {
    ...
    const input = createTestInput(
      respText,
      gtText,
      configWithCustomModel.criterionName
    );
  ...
  it('should correctly apply different thresholds (for binary scale)', async () => {
    ...
    const vecProducing07_2 = [
      ...
      0,
      0
    ]; // Results in cosine of 0.7 with vec1
    ...
    input = createTestInput(
      ...
      baseEvalConfig.criterionName,
      'binary'
    ); // Recreate input if needed, or ensure criteria name matches
  ...
  it('should output raw similarity score and mention threshold when scale is numeric', async () => {
    ...
    const vecProducing085_2 = [
      0.85,
      Math.sqrt(1 - 0.85 * 0.85) * (vecProducing085_1[1] > 0 ? 1 : -1) * 0.5 +
        vecProducing085_1[1],
      0,
      0,
      0,
