...
const deepResearchParamsSchema = z.object({
  ...
  depth: z
    ...
    .max(3)
    .default(1)
    .describe('Depth of research (1-3)'),
  breadth: z
    .number()
    .int()
    .min(3)
...
function safelyHandleError(error: unknown, context: string): void {
  logger.error(LogCategory.NODE, '[DeepResearch]', `Error in ${context}:`, {
    error
  });
  throw new Error(
...
async function getDeepContent(
  ...
  for (let batchIndex = 0; batchIndex < batches; batchIndex++) {
    ...
    const batchPromises = currentBatch.map(async (source) => {
      try {
        ...
        if (
          scrapeContent.includes('firecrawl_error') ||
          scrapeContent.includes('API Error')
        ) {
          logger.warn(
            LogCategory.NODE,
            '[DeepResearch]',
  ...
  if (errorCount === sourcesToScrape.length && sourcesToScrape.length > 0) {
    throw new Error(
      `Failed to scrape content from all ${sourcesToScrape.length} sources`
    );
  }
...
${cleanContent.substring(0, 18000)}`;
      ...
      const llmFindings = result.text
        ...
        .replace(
          /^(key findings|findings|research findings|summary|analysis):?\s*/i,
          ''
        )
        .replace(
          /^(here are|the following are|based on the research)\s+.*?:\s*/i,
          ''
        )
        // Split by lines
        .split('\n')
...
export const deepResearchTool: Tool = {
  name: 'deep_research',
  description:
    'Perform in-depth research on a topic with search and content extraction',
  parameters: deepResearchParamsSchema,
  async execute({ query, depth = 2, breadth = 5 }, options) {
    logger.debug(
      LogCategory.NODE,
      '[DeepResearch]',
      `Executing research for query: ${query}`,
    ...
    try {
      ...
      const { findings: llmAnalyzedFindings, status: analysisStatus } =
