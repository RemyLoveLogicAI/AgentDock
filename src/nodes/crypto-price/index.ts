...
const cryptoPriceSchema = z.object({
  ...
  currency: z
    .string()
    .optional()
    .describe('Currency to display price in (e.g., usd, eur, btc)'),
  apiKey: z
    .string()
    .optional()
    .describe(
      'Optional CoinGecko API key (will use environment variable if not provided)'
    )
});
...
export const cryptoPriceTool: Tool = {
  name: 'crypto_price',
  description:
    'Get the current cryptocurrency price and market data for a given coin ID or symbol',
