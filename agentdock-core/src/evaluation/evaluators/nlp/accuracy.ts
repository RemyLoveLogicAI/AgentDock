...
export class NLPAccuracyEvaluator implements Evaluator {
  ...
  constructor(config: NLPAccuracyEvaluatorConfig) {
    ...
    if (!config.embeddingModel) {
      throw new Error(
        '[NLPAccuracyEvaluator] embeddingModel must be provided.'
      );
    }
    this.config = config;
  }
  ...
  async evaluate(
    ...
    const responseText =
      typeof input.response === 'string'
        ? input.response
        : JSON.stringify(input.response); // Simple stringification for AgentMessage
    const groundTruthText = String(input.groundTruth); // Ensure groundTruth is a string

    try {
      const [responseEmbeddingResult, groundTruthEmbeddingResult] =
        await Promise.all([
