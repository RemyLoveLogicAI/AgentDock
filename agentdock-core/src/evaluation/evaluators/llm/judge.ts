...
export class LLMJudgeEvaluator implements Evaluator {
  ...
  constructor(config: LLMJudgeConfig) {
    ...
    if (
      ...
      throw new Error(
        '[LLMJudgeEvaluator] promptTemplate must be a non-empty string.'
      );
    }
    if (
      ...
      throw new Error(
        '[LLMJudgeEvaluator] criterionName must be a non-empty string.'
      );
    }
    this.config = config;
  }
    ...
    switch (scale) {
      ...
      case 'likert5':
        return z
          ...
          .min(1)
          .max(5)
          .describe('Score as integer between 1 and 5.');
      case 'numeric':
  ...
  async evaluate(
    ...
    const llmResponseSchema = z.object({
      ...
      reasoning: z
        .string()
        .optional()
        .describe('The reasoning behind the score.')
    });
        ...
        return [
          {
            ...
            metadata: {
              rawLlmScore: llmOutput.score,
              rawLlmReasoning: llmOutput.reasoning
            }
          }
        ];
      }
  ...
  private preparePrompt(
    ...
    prompt = prompt
      .split('{{criterion_description}}')
      .join(criterion.description);
    ...
    switch (scale) {
      ...
      case 'numeric': {
        if (rawValue === null || rawValue === undefined) {
          ...
          return {
            error: `Invalid value for numeric scale: ${rawValue}. Expected a number.`
          };
        }
