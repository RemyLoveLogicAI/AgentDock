...
import { LLMConfig, LLMMessage, LLMProvider, TokenUsage } from '../llm/types';
...
export class AgentNode extends BaseNode<AgentNodeConfig> {
  ...
  constructor(id: string, config: AgentNodeConfig) {
    ...
    if (!config.apiKey) {
      throw createError(
        'node',
        'API key is required for AgentNode',
    ...
    if (
      ...
      const fallbackProviderOrDefault =
        config.fallbackProvider || this.llm.getProvider();
      const fallbackModelOrDefault =
        config.fallbackModel || 'claude-3-5-sonnet-20240620';
  ...
  private createLLMInstance(
    ...
    try {
      ...
      logger.debug(
        LogCategory.NODE,
        'AgentNode',
        'Attempting to derive provider',
        { nodeId: this.id, initialProvider: determinedProvider }
      );
      ...
      if (!modelName && this.config.agentConfig) {
        const providerNodeType =
          ProviderRegistry.getNodeTypeFromProvider(determinedProvider);
        modelName =
          this.config.agentConfig.nodeConfigurations?.[providerNodeType]?.model;
      }
      if (!modelName) {
      ...
      logger.debug(LogCategory.NODE, 'AgentNode', 'Derived model', {
        nodeId: this.id,
        modelName
      });
      ...
      logger.debug(
        LogCategory.NODE,
        'AgentNode',
        'Calling actual createLLM function',
        { nodeId: this.id, provider: determinedProvider, model: modelName }
      );
      ...
      logger.debug(
        LogCategory.NODE,
        'AgentNode',
        'Attempting to return LLM instance',
        {
          nodeId: this.id,
          instanceDetails: {
            provider: llmInstance?.getProvider
              ? llmInstance.getProvider()
              : 'unknown',
            model: llmInstance?.getModelId
              ? llmInstance.getModelId()
              : 'unknown'
          }
        }
      );
      ...
      logger.error(
        LogCategory.NODE,
        'AgentNode',
        'IMMEDIATE CATCH in createLLMInstance',
        {
      ...
      logger.error(
        LogCategory.NODE,
        'AgentNode',
        'Error caught within createLLMInstance',
        {
          nodeId: this.id,
          provider,
  ...
  private async getAvailableTools(
    ...
    try {
      ...
      const allAgentToolsList: Tool[] = Object.entries(allAgentToolsObj).map(
        ([toolKey, tool]) => ({ ...tool, name: toolKey })
      );
      const allToolIds = allAgentToolsList.map((t) => t.name);

      const orchestrationConfig = this.config.agentConfig?.orchestration || {
        steps: []
      };
  ...
  async handleMessage(
    ...
    const activeLLM =
      useFallback && this.fallbackLlm ? this.fallbackLlm : this.llm;
    if (!activeLLM) {
      throw createError(
        'node',
    ...
    try {
      ...
      let currentOrchestrationState: AIOrchestrationState | null =
        providedState || null;
      if (!currentOrchestrationState) {
        logger.debug(
          LogCategory.NODE,
          'AgentNode',
          'Fetching orchestration state for prompt',
          { nodeId: this.id, sessionId: sessionId?.substring(0, 8) }
        );
        currentOrchestrationState =
          await orchestrationManager.getState(sessionId);
      }
      ...
      logger.debug(
        LogCategory.NODE,
        'AgentNode',
        'Dynamic state prepared for prompt',
        {
          nodeId: this.id,
          sessionId: sessionId?.substring(0, 8),
      ...
      if (systemOverride) {
        ...
        finalSystemPrompt = createSystemPrompt(
          tempAgentConfig,
          dynamicStateForPrompt
        );
      } else {
        // Pass dynamic state as second argument
        finalSystemPrompt = createSystemPrompt(
...
ISO timestamp: ${now.toISOString()}`;
      ...
      logger.debug(
        LogCategory.NODE,
        'AgentNode',
        'Final System Prompt before LLM call',
        {
          nodeId: this.id,
          sessionId: sessionId?.substring(0, 8),
      ...
      logger.debug(
        LogCategory.NODE,
        'AgentNode',
        'Calling LLM Orchestration Service',
        {
          nodeId: this.id,
          sessionId: sessionId?.substring(0, 8),
      ...
      const result = await orchestrationService.streamWithOrchestration({
        ...
        maxSteps: maxSteps,
        temperature:
          runtimeOverrides?.temperature ?? this.config.options?.temperature,
        maxTokens:
          runtimeOverrides?.maxTokens ?? this.config.options?.maxTokens,
      ...
      logger.debug(
        LogCategory.NODE,
        'AgentNode',
        'Returning stream result object from service',
        { nodeId: this.id, sessionId: sessionId?.substring(0, 8) }
      );
  ...
  protected getDescription(): string {
    return (
      this.config?.agentConfig?.description ||
      'Core Agent Node using Vercel AI SDK streamText.'
    );
  }
  ...
  protected getInputs(): readonly NodePort[] {
    return [
      ...
      {
        id: 'config',
        type: 'Partial<LLMConfig>',
        label: 'LLM Config Overrides',
        required: false
      },
      {
