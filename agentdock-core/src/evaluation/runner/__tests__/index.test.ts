...
describe('runEvaluation function', () => {
  ...
  describe('Run Scenarios and Aggregation', () => {
    it('should aggregate results from multiple evaluators', async () => {
      ...
      const result: AggregatedEvaluationResult = await runEvaluation(
        input,
        runConfig
      );

      expect(result.results).toHaveLength(2);
      const qualityRes = result.results.find(
        (r) => r.criterionName === 'Quality'
      );
      const safetyRes = result.results.find(
        (r) => r.criterionName === 'Safety'
      );
      ...
      if (sumOfWeights > 0)
        expectedOverallScore = totalWeightedScore / sumOfWeights;
    ...
    */
        ...
        evaluatorConfigs: [
          {
            type: 'Toxicity',
            config: {
              criterionName: 'NonExistentCriterionInInput',
              toxicTerms: ['fail']
            }
          }
        ],
    ...
    it('should correctly use weights in overallScore calculation', async () => {
      ...
      expect(result.results).toHaveLength(2);
      const qualityScoreObj = result.results.find(
        (r) => r.criterionName === 'Quality'
      );
      const safetyScoreObj = result.results.find(
        (r) => r.criterionName === 'Safety'
      );
    ...
    it('should handle multiple criteria assessed by separate NLPAccuracy configs', async () => {
      ...
      const clarityRes = result.results.find(
        (r) => r.criterionName === 'Clarity'
      );
      ...
      if (sumOfWeights > 0)
        expectedOverallScoreInTest = totalWeightedScore / sumOfWeights;
    ...
    it('should correctly use default weight of 1 if weight is not specified in criteria', async () => {
      ...
      expect(result.results).toHaveLength(2);
      const clarityScoreObj = result.results.find(
        (r) => r.criterionName === 'Clarity'
      );
      const safetyScoreObj = result.results.find(
        (r) => r.criterionName === 'Safety'
