...
import {
  ...
  UpstashRedisStorageProviderConfig
...
describe('RedisStorageProvider', () => {
  ...
  describe('Batch Operations', () => {
    ...
    describe('getMany', () => {
      it('should get multiple existing values', async () => {
        ...
        expect(mockRedisClientInstance.mget).toHaveBeenCalledWith(
          ...namespacedKeys
        );
  ...
  describe('List/Clear Operations', () => {
    ...
    it('list() should return keys matching a prefix using scan', async () => {
      ...
      expect(mockRedisClientInstance.scan).toHaveBeenCalledWith(0, {
        match: listPattern,
        count: 100
      });
    ...
    it('list() should handle multiple scan iterations', async () => {
      ...
      expect(mockRedisClientInstance.scan).toHaveBeenNthCalledWith(1, 0, {
        match: listPattern,
        count: 100
      });
      expect(mockRedisClientInstance.scan).toHaveBeenNthCalledWith(2, '123', {
        match: listPattern,
        count: 100
      });
      expect(result).toEqual(keysToReturn);
    ...
    it('list() should return empty array on error or no keys', async () => {
      ...
      mockRedisClientInstance.scan.mockRejectedValueOnce(
  ...
  describe('List Type Operations', () => {
    ...
    it('getList() should retrieve a range of values using lrange', async () => {
      ...
      expect(mockRedisClientInstance.lrange).toHaveBeenCalledWith(
        namespacedListKey,
        0,
        -1
      );
    ...
    it('getList() should return null on error', async () => {
      mockRedisClientInstance.lrange.mockRejectedValue(
        new Error('LRANGE failed')
      );
