  ...
  try {
    ...
    if (stats.isDirectory()) {
      ...
      for (const file of markdownFiles) {
        ...
        const title =
          extractTitle(content) || formatTitle(file.replace('.md', ''));
      ...
      const title =
        ...
        formatTitle(path.basename(basePath).replace('.md', ''));
...
export function getPrevNextPages(
  ...
  if (currentIndex === -1) {
    console.warn(
      `[getPrevNextPages] Current path '${currentPath}' not found in flattened list.`
    );
