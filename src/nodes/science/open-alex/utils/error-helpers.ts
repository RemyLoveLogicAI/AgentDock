...
export function handleApiError(error: unknown): string {
  ...
  if (error && typeof error === 'object' && 'status' in error) {
    ...
    if (fetchError.status === 429) {
      logger.error(LogCategory.NODE, '[OpenAlexAPI]', 'Rate limit exceeded', {
        error: fetchError
      });
    ...
    if (fetchError.status === 404) {
      logger.error(LogCategory.NODE, '[OpenAlexAPI]', 'Resource not found', {
        error: fetchError
      });
    ...
    if (fetchError.status >= 500) {
      logger.error(LogCategory.NODE, '[OpenAlexAPI]', 'Server error', {
        error: fetchError
      });
  ...
  logger.error(LogCategory.NODE, '[OpenAlexAPI]', 'Unknown error type', {
    error: errorStr
  });
