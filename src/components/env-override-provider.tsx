    ...
    try {
      ...
      if (process.env.NEXT_PUBLIC_DEFAULT_HISTORY_POLICY) {
        ...
        if (['none', 'lastN', 'all'].includes(envPolicy)) {
          ...
          console.info(
            `[EnvOverride] Set history policy from environment: ${envPolicy}`
          );
        }
      }

      if (process.env.NEXT_PUBLIC_DEFAULT_HISTORY_LENGTH) {
        const envLength = parseInt(
        ...
        if (!isNaN(envLength) && envLength >= 0) {
          ...
          console.info(
            `[EnvOverride] Set history length from environment: ${envLength}`
          );
        }
      }
      ...
      if (hasParams) {
        ...
        if (
          ...
          console.info(
            `[EnvOverride] Set history policy from URL: ${historyPolicy}`
          );
        }
        ...
        if (historyLength !== null && !(window as any).ENV_HISTORY_LENGTH) {
          ...
          if (!isNaN(length) && length >= 0) {
            ...
            console.info(
              `[EnvOverride] Set history length from URL: ${length}`
            );
          }
        }
      ...
      console.debug('[EnvOverride] Final environment settings:', {
        ...
        source: process.env.NEXT_PUBLIC_DEFAULT_HISTORY_LENGTH
          ? 'environment'
          : 'URL'
      });
    } catch (error) {
      console.error(
        '[EnvOverride] Failed to set environment overrides:',
        error
      );
    }
  }, []);
