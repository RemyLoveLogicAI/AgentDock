...
export function createSystemPrompt(
  ...
  if (agentConfig.orchestration) {
    ...
    finalPrompt = addOrchestrationToPrompt(
      finalPrompt,
...
function addDynamicStateToPrompt(
  ...
  if (
    dynamicState.recentlyUsedTools &&
    dynamicState.recentlyUsedTools.length > 0
  ) {
 ...
 */
function generateDetailedOrchestrationGuide(
  orchestration: OrchestrationConfig
): string {
  let guide = '';
  ...
  orchestration.steps.forEach((step, index) => {
    ...
    if (step.availableTools) {
      ...
      if (
        ...
        step.availableTools.allowed.length > 0
      ) {
 ...
 */
function generateCompactOrchestrationGuide(
  orchestration: OrchestrationConfig
): string {
  let guide = '';
