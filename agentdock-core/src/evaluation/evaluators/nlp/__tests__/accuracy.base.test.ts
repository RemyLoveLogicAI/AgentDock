...
describe('NLPAccuracyEvaluator - Base Functionality', () => {
  ...
  const evaluatorConfigForConstructor: NLPAccuracyEvaluatorConfig = {
    ...
    embeddingModel:
      'text-embedding-3-small' as unknown as EmbeddingModel<string>, // Use type assertion via unknown
  ...
  it('should correctly score (false for binary, raw for numeric) when similarity is below threshold', async () => {
    ...
    expect(expectedLowSimilarity).toBeLessThan(
      evaluatorConfigForConstructor.similarityThreshold!
    ); // 0 < 0.8, should pass
