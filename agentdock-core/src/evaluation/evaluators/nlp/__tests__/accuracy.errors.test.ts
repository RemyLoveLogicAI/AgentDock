...
describe('NLPAccuracyEvaluator - Error Handling', () => {
  ...
  it('should handle embedding generation failure for response', async () => {
    ...
    expect(result.score).toBe(
      input.criteria![0].scale === 'binary' ? false : 0
    );
  ...
  describe('Constructor Error Handling', () => {
    ...
    it('should throw error if criterionName is missing in config', () => {
      ...
      expect(
        () =>
          new NLPAccuracyEvaluator(
            configWithoutCriterionName as NLPAccuracyEvaluatorConfig
          )
      ).toThrow(
    ...
    it('should throw error if criterionName is an empty string in config', () => {
      ...
      expect(
        () => new NLPAccuracyEvaluator(configWithEmptyCriterionName)
      ).toThrow(
        '[NLPAccuracyEvaluator] criterionName must be a non-empty string.'
      );
    });
    ...
    it('should throw error if embeddingModel is missing in config', () => {
      ...
      expect(
        () =>
          new NLPAccuracyEvaluator(
            configWithoutModel as NLPAccuracyEvaluatorConfig
          )
